...
- **El panel es público a propósito, y eso NO se cierra.** Está linkeado desde `/agenda/destacado` con la frase "los números del sitio están abiertos en un panel público": es el argumento de venta del espacio pago, porque una marca puede verificar sola en vez de creernos. Incluye las 200 consultas de Search Console, que es nuestra investigación de palabras clave; se revisó el 25/9/2026 y **se decidió dejarlo abierto entero**. No hay riesgo de datos personales: la tabla `hits` no guarda IP, cookie ni huella, y por eso tampoco hace falta banner de cookies.
- **`/api/registrar` escribe de a tandas, no en cada pedido.** Recibe ~1500 pedidos por día, uno por minuto, y Neon cobra por horas de base PRENDIDA: con un pedido por minuto no se apagaba nunca y el 24/9/2026 se quemó la cuota (110 CU-hrs de 100) y el panel se cortó entero. Ahora junta y escribe cada 100 eventos o cada media hora. **No volver a escribir fila por fila**, y si se toca, el `momento` va explícito en el INSERT: con el `DEFAULT now()` de la tabla, los cien eventos de una tanda quedan con la hora del volcado y los gráficos por hora muestran picos que nunca existieron.
...
6bis. **Un evento tampoco.** El slug de una ficha sale del campo `Slug` de Airtable. Si lo cambiás a mano, la dirección vieja queda en la nada: hay links compartidos y resultados de Google que todavía la usan. Poné la vieja en el campo **`Slugs anteriores`** (una por línea) y el sitio redirige solo. El panel ya congela el slug al renombrar; el agujero es editar Airtable directo. Los registros de Vercel del 5/9/2026 mostraron tres direcciones muertas comiéndose ~15 visitas por día, y ninguna daba señal en ningún lado hasta que se miraron.
